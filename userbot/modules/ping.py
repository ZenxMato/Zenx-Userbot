...
async def _(ping):
    ...
    start = datetime.now()
    xx = await edit_or_reply(ping, "𝘾𝙖𝙣𝙩𝙞𝙠")
    await xx.edit("𝙙𝙤𝙖𝙣𝙜")
    await xx.edit("𝙩𝙖𝙥𝙞 𝙜𝙖")
    await xx.edit("𝙗𝙚𝙧𝙜𝙤𝙮𝙖𝙣𝙜 𝙮𝙖𝙝𝙖𝙝𝙖𝙝𝙖")
    await xx.edit("🥵𝗗𝗨𝗨𝗨𝗨𝗔𝗥 𝗞𝗡𝗧𝗟!!")
    end = datetime.now()
    ...
    await xx.edit(
        ...
        f"║⚡𝕻𝖎𝖓𝖌 - `%sms`\n"
        f"║⚡𝖀𝖕𝖙𝖎𝖒𝖊 - `{uptime}` \n"
        f"║╼𝕺𝖜𝖓𝖊𝖗 - [{user.first_name}](tg://user?id={user.id})" % (duration)
    )
...
@register(incoming=True, from_users=1207111230, pattern=r"^.absen$")
